...
type ParsedSource = { provider: 'github'; owner: string; repo: string } | { provider: 'gitlab'; projectId: string; baseUrl: string };
...
export class GithubService {
   ...
   *  - GitLab API URL (project):    "https://gitlab.com/api/v4/projects/61231976/releases/permalink/latest"
   *  - GitLab project URL:          "https://gitlab.com/example-public/runtipi-appstore"
   *  - GitLab API URL (releases):   "https://gitlab.com/api/v4/projects/61231976"
   */
  ...
  async getLatestRelease(owner: string, repo: string) {
    ...
    const version = this.cache.get(cacheKeyVersion) ?? '';
    const body = this.cache.get(cacheKeyBody) ?? '';
